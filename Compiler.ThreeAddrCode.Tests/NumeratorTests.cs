...
namespace Compiler.ThreeAddrCode.Tests
{
    ...
    public class NumeratorTests
    {
        ...
        {
            var code = GetTestGraph1();
            var graph = code.BackOrder();

            var nodes = graph.CFGNodes.ToArray();
            for (int i = 0; i < nodes.Length; i++)
                Assert.AreEqual(graph.IndexOf(nodes[i]), nodes.Length - 1 - i);
        }
    }
}